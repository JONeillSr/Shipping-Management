...
.PARAMETER OutputDirectory
    Directory where the generated HTML/PDF files will be saved. Defaults to <Config folder>\Output if -ConfigPath is provided; otherwise current directory.

.PARAMETER TemplatePath
    Optional path to a tokenized HTML template (e.g., Templates\LogisticsEmail_Template_New.html). If omitted, the script uses template_path from the JSON config when available.
    Directory containing lot images (format: lotnumber.jpg)
...
.NOTES
    ...
    Version: 1.7.0
    Change Date: 10/29/2025
    Change Purpose: Fixed all HTML parsing errors using proper here-strings

.CHANGELOG
    1.7.0 - 10/29/2025 - Added -TemplatePath and -OutputDirectory; switched to template-driven HTML rendering compatible with JSON config.
...
function New-LogisticsEmailHTML {
    ...
    try {
        # Resolve fields from config or defaults
        ...
        $deliveryAddress = if ($Config -and $Config.delivery_address) { $Config.delivery_address } else { "[Delivery address not specified]" }
        ...
        if (-not $TemplatePath -and $Config -and $Config.template_path) {
            $TemplatePath = $Config.template_path
        }
        if (-not $TemplatePath) {
            $TemplatePath = Join-Path $PSScriptRoot "Templates\LogisticsEmail_Template_New.html"
        }
        ...
        foreach ($k in $replacements.Keys) {
            $html = $html.Replace($k, [string]$replacements[$k])
        }
        return $html
    } catch {
        throw $_
